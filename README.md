# PeroseMart — Multi-Seller E-Commerce Marketplace
...
| :--- | :--- | :--- | :--- |
| **ADMIN** | `admin@perosemart.com` | `admin123` | Platform console, user governance, order moderation |
| **SELLER** | `seller1@perosemart.com` | `seller123` | Seller hub, inventory management, order dispatch |
| **SELLER** | `seller2@perosemart.com` | `seller123` | Seller hub (BookWorld Store listings) |
| **BUYER** | `buyer1@perosemart.com` | `buyer123` | Catalog browse, shopping cart, checkout, reviews |
| **BUYER** | `buyer2@perosemart.com` | `buyer123` | Catalog browse, shopping cart, checkout, reviews |
...
```
The packaged deployable WAR will be created at `target/perosemart.war`.

### Running with Embedded Tomcat (Development & Testing)
Automated tests automatically launch embedded Apache Tomcat 9 at `http://localhost:8888/perosemart` during test suite execution.

### Deploying to Standalone Apache Tomcat 9
1. Copy `target/perosemart.war` to `$CATALINA_HOME/webapps/perosemart.war`.
2. Start Tomcat:
...
3. Access the web application in any browser at:  
   `http://localhost:8080/perosemart/`
